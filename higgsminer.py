...
def main():
    ...
    #gbc = GBC(n_estimators=50, max_depth=10,min_samples_leaf=200,max_features=15,verbose=1, learning_rate=0.05, subsample=0.5)
    #gbc = GBC(n_estimators=50, max_depth=10,min_samples_leaf=200,max_features=10,verbose=1)
    #gbc = GBC(n_estimators=1600, verbose=1)
    gbc = GBC(n_estimators=100, max_depth=10, min_samples_leaf=200, max_features=15, verbose=1)
    ...
    #gbc = tree.DecisionTreeClassifier(max_features=15)
    ...
    prob_predict_train = gbc.predict_proba(X_train)[:,1] # column corresponds to Y_train being 1
...
def load_training_data():
    ...
    # First 90% (r < 0.9 selects 90% of the datapoints randomly) are training
